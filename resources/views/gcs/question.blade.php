  ...
  <div id="spinner" class="spinner"></div> {{-- ローディングスピナー用のタグ --}}
    <section class="lg:w-1/2 text-gray-900 body-font">
        <div class="container px-5 lg:py-40 py-12 mx-auto">
            <h1 class="ml-2 text-4xl"><span class="text-green-500 font-bold">A</span>sk a Question About GC Analysis Data</h1>
            <div class="ml-2 h-1 w-20 bg-green-500 rounded"></div>
            <br>
            <form action="{{ route('searchGc') }}" method="POST" onsubmit="showSpinner()">
                @csrf
                <label class="ml-2 text-gray-600 " for="query">Enter your question:</label>
                <textarea class="ml-2 pl-2 w-full min-h-40 rounded-lg border border-green-500 text-lg" type="text" id="query" name="query" required></textarea>
        ...
        <div class="container px-5 lg:py-10 py-12 mx-auto">
          ...
          <div class="flex items-center lg:w-4/5 mx-auto border-b pb-10 mb-10 border-gray-200 sm:flex-row flex-col">
            ...
            <div class="sm:w-32 sm:order-none order-first sm:h-32 h-20 w-20 sm:ml-10 inline-flex items-center justify-center rounded-full bg-green-100 text-green-500 flex-shrink-0">
                <div class="text-6xl fa-regular fa-face-smile-beam" style="color: #29e01f;"></div>           
            </div>
          </div>
