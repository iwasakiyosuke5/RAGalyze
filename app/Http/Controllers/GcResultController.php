...
class GcResultController extends Controller
{
    /**
    ...
    {
        // 検索できるようした
        $query = GcResult::where('user_id', auth()->id());
        ...
        return view('gcs.myPosts', compact('mines'));
        // // 下記は検索しない場合
        // $mines = GcResult::where('user_id', auth()->id())→orderBy('updated_at','desc')->paginate(5);
    }
    ...
    public function show($id, Request $request)
    {
        ...
        $post = GcResult::where('id', $id)->firstOrFail(); 
        ...
        return view('gcs/myPosts', compact('mines', 'post', 'textData', 'imageData'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(GcResult $gcResult)
    {
        //
    }
     ...
     */
    public function update(Request $request, GcResult $gcResult)
    {
        $request->validate([
        ...
        // GcResult::where('id', $gcResult->id)->update([
        //     'content' => $text,
        //     'vector' => json_encode($vector),
        // ]);

        return redirect()->route('gcMyPosts')->with('success', 'データが更新されました。'); // マイページにリダイレクト
     ...
     */
    public function destroy(GcResult $gcResult, Request $request)
    {
        ...
        return redirect()->route('gcMyPosts')->with('success', 'データが削除されました。'); 
    }
  //request.blade.phpからwatch.bladeに遷移 
    ...
    private function sendToOpenAI($imageData, $filePath)
    {
            ...
            'messages' => [
                [
                    ...
                    'content' => [
                        [
                            'type' => 'text',
                            'text' => "Please analyze the provided GC chromatogram image and categorize the information into the following sections:\n\n"
        ...
        return view('gcs/watch', ['textData' => $textData, 'imageData' => $imageData, 'filePath' => $filePath]);
    }

  //watch.bladeからデータを登録する時の流れ  
    //watch.bladeでgc_resultsテーブルにデータを保存
    public function upload(Request $request)
    {
        ...
        return redirect('gcs/request')->with('success', 'データがアップロードされました。');
    }
  ...
  // watch.blade.phpで画像ファイルを削除して再度request.bladeに戻る関数
    public function deleteAndRedirect(Request $request)
    {
        ...
        return redirect()->route('gcRequest');
    }
